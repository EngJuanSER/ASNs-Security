...
\begin{itemize}
    \item \textbf{Nmap:} Herramienta de código abierto para escaneo de red y detección de servicios~\cite{lyon2009nmap}. Se propone utilizar TCP Connect scan (-sT) en lugar de SYN scan por compatibilidad con entornos cloud que bloquean raw sockets. Soporta detección de versiones (-sV) mediante fingerprinting activo de respuestas de servicios.
...
Para el análisis de geolocalización se propone utilizar la base de datos GeoLite2 de MaxMind. Schopman~\cite{schopman2021validating} validó la precisión de GeoLite2 City database mediante análisis de 100,000+ direcciones IP con ubicaciones verificadas, encontrando que mantiene niveles aceptables de precisión para uso académico y aplicaciones no críticas:
\begin{itemize}
...
\textbf{Proceso de Correlación Propuesto:}
...
Clasificación de activos según niveles de riesgo agregado (escala estándar 0-10):
\begin{itemize}
...
El desafío en entornos reales no es solo detectar vulnerabilidades sino priorizarlas efectivamente. Se propone un algoritmo de scoring que considere:
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Configuración propuesta: TCP connect scan (-sT) para compatibilidad con entornos cloud
        \item Comando base: \texttt{nmap -sT -sV -p [ports] --open -T4 -oX [output] [target]}
        \item Detección de servicios y versiones mediante fingerprinting activo
        \item Output XML parseado mediante librerías estándar del lenguaje
    ...
    \begin{itemize}
        ...
        \item Descarga automática durante proceso de build con herramientas estándar
        \item Almacenamiento: directorio de recursos del backend
        \item Actualizaciones mensuales disponibles
        \item Consultas: $<$1ms con librerías nativas de lectura MMDB
...
\textbf{Backend:}
\begin{itemize}
    \item Lenguaje: Java (versión LTS reciente) por su robustez y ecosistema maduro
    \item Framework: Quarkus, optimizado para aplicaciones cloud-native con bajo consumo de recursos
    \item Arquitectura: API REST stateless para facilitar escalabilidad y despliegue
    \item Build: Gradle o Maven para gestión de dependencias
\end{itemize}

\textbf{Frontend:}
\begin{itemize}
    \item Framework: Vue.js 3 con Composition API para interfaces reactivas
    \item Lenguaje: TypeScript para tipado estático y mejor mantenibilidad
    \item Visualizaciones: Librerías de gráficos (Chart.js) y mapas (Leaflet.js)
    \item Cache local: IndexedDB mediante LocalForage para persistencia en navegador
\end{itemize}

\textbf{Infraestructura:}
\begin{itemize}
    \item Contenedores: Docker para empaquetado consistente
    \item Despliegue: Plataformas cloud con tier gratuito (Railway, Vercel, Render)
