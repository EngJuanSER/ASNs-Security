...
\textbf{GeoLite2 MaxMind:} Se utiliza la base de datos GeoLite2-City en formato MMDB (MaxMind database, 61MB comprimido), descargada automáticamente durante el proceso de build de Docker desde el repositorio GitHub P3TERX/GeoLite.mmdb mediante curl. La base de datos se almacena en \texttt{src/main/resources/geo/} y se accede mediante la librería \texttt{maxmind-db-reader} de Java. Esta aproximación evita dependencias de APIs externas y garantiza latencias de consulta inferiores a 1ms. La actualización manual mensual se realiza rebuildeando la imagen Docker.

\textbf{Nmap Network Scanner:} El sistema ejecuta Nmap mediante \texttt{ProcessBuilder} de Java para escaneo activo de puertos y detección de servicios. Debido a restricciones de Railway que bloquean raw sockets, se utiliza TCP connect scan (-sT) en lugar de SYN scan (-sS). El flujo de ejecución implementado incluye:
\begin{itemize}
    \item Construcción del comando: {\small \texttt{nmap -sT -sV -p [ports] --open -T4 -oX [output] [target]}} para escaneo con TCP connect y detección de versiones
    \item Ejecución asíncrona del proceso con timeout configurable (60 segundos por defecto)
    \item Parsing del XML de salida mediante DocumentBuilder (DOM parser de Java)
    \item Extracción automatizada de puertos abiertos, nombres de servicios, versiones de software y strings CPE
    \item El TCP connect scan agrega 20-30\% de latencia vs SYN scan pero no requiere privilegios de root
\end{itemize}
...
\textbf{National Vulnerability Database (NVD):} El sistema accede a la API REST pública de NIST en 

\url{https://services.nvd.nist.gov/rest/json/cves/2.0} 

para búsqueda de vulnerabilidades. El proceso de correlación implementado incluye:
\begin{itemize}
    \item Extracción automática de CPE strings de la salida de Nmap (formato: {\small \texttt{cpe:2.3:a:vendor:product:version:*}})
    \item Ejecución de queries a NVD API filtrando por CPE exacto: {\small \texttt{GET /cves/2.0?cpeName=<cpe>}}
...
\textbf{ipapi.co API:} El sistema complementa los datos de GeoLite2 consultando ipapi.co para obtener información actualizada de ASN, ISP y organización propietaria de la red. Esta API REST pública (\url{https://ipapi.co/[ip]/json}) permite 1000 requests diarios sin autenticación. La integración proporciona contexto de red más preciso que WHOIS manual y enriquece el análisis con metadatos actualizados sobre el propietario de la IP.
